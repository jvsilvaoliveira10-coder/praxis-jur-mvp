...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Use anon key for queries (data is public via RLS)
    ...
    console.error('Error in search-legal-references:', error instanceof Error ? error.message : error);
    return new Response(
      JSON.stringify({
        success: false,
        error: 'Erro interno ao buscar referências legais'
      }),
